...
void Pipeline::ResetDetPredictor(const std::string &detModelDir,
...
cv::Mat Pipeline::Process(cv::Mat img,
...
void Pipeline::Process(cv::Mat img, std::vector<DetectionResult>& results) {
    //  Timer tic;
    //  tic.start();
    int use_direction_classify = int(Config_["use_direction_classify"]); // NOLINT
    cv::Mat srcimg;
    ...
    for (int i = boxes.size() - 1; i >= 0; i--) {
        ...
        DetectionResult r;
        std::vector<std::vector<int>> box = boxes[i];
        int x_collect[4] = {box[0][0], box[1][0], box[2][0], box[3][0]};
        int y_collect[4] = {box[0][1], box[1][1], box[2][1], box[3][1]};
        int left = int(*std::min_element(x_collect, x_collect + 4));   // NOLINT
        int right = int(*std::max_element(x_collect, x_collect + 4));  // NOLINT
        ...
        r.box = cv::Rect{left, top, right - left, bottom - top};
        r.text = rec_text.back();
        r.text_score = rec_text_score.back();
        results.push_back(r);
    }
